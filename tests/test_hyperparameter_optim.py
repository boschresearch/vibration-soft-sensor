...
from softsensor.losses import (
    DistributionalMSELoss,
    GaussianNLLLoss,
    BetaNLL,
    PinballLoss,
    PSDLoss,
)
from softsensor.hyperparameter_optimization import (
    grid_search,
    hyperopt_search,
    random_search,
)
...
def test_random_search():
    ...
    df, model = random_search(
        handler, criterion, model_type, model_params, grid_params, val_prediction=True
    )
    ...
    df, model = random_search(
        ...
        grid_params,
        val_prediction=True,
        val_criterion=GaussianNLLLoss(),
    )

    grid_params = {"lr": stats.loguniform(1e-5, 1e-3), "optimizer": ["Adam", "SGD"]}
    ...
    df, model = random_search(
        ...
        reconfigure_criterion=True,
        val_prediction=True,
        val_criterion=GaussianNLLLoss(),
    )

    grid_params = {"lr": stats.loguniform(1e-6, 1e-4), "optimizer": ["Adam", "SGD"]}
    ...
    df, model = random_search(
        handler, PinballLoss(quantiles), model_type, model_params, grid_params
    )
    ...
    df, model = random_search(
        ...
        grid_params,
        val_prediction=True,
        val_criterion=PinballLoss(quantiles),
    )
    ...
    df, model = random_search(
        handler, criterion, model_type, model_params, grid_params, val_prediction=False
    )
...
def test_grid_search():
    ...
    df, model = grid_search(
        handler, criterion, model_type, model_params, grid_params, val_prediction=True
    )
    print(df["loss"])

    grid_params = {"lr": [0.000001, 0.00001], "optimizer": ["Adam", "SGD"]}
...
def test_calibration():
    ...
    grid_params = {"lr": [0.0001, 0.001], "optimizer": ["Adam", "SGD"]}
...
def test_comp_results():
    results = {"train_loss": [5, 1, 2], "val_loss": [], "stability_score": []}
    res = _comp_results(results)
    assert res["loss"] == 1

    results = {"train_loss": [5, 1, 2], "val_loss": [2, 2, 6], "stability_score": []}
    res = _comp_results(results)
    assert res["loss"] == 2
